...
def get_idle_timeout(cmd):
    ...
    return COMMAND_DELAYS.get(cmd,0.25)
...
class PSXMonitor:
    ...
    def upload_program(self, filename):
        ...
        self.execute_program(self.ser, exe.entry)
    ...
    def execute_program(self, ser, entry_point):
        """
...
class Proxy:
    ...
    def __init__(self, monitor, listen):
    ...
    def run(self):
        ...
        while True:
            ...
            response=self.monitor.send(data)
...
#
# Convenience reverse engineering commands
#
...
class PSX:
    ...
    def help(self):
        return self.mon.send("help")

    def regs(self):
        return self.mon.send("dr")


    def status(self):
        return self.mon.send("ds")


    def disasm(self,address,count=16):

        # current monitor syntax unknown
        # likely:
        # di address length

        cmd=f"di {address:x} {count}"

        return self.mon.send(cmd)


    def dump_word(self,address,count):

        cmd=f"dw {address:x} {count}"
...
def main():
    ...
    if args.proxy:
        ...
        while True:
            ...
            if cmd == "test":
                mon.test_bwr_write(0x800F0000,"test.bin")
                continue

            if cmd.startswith("run "):
                _, file = cmd.split(maxsplit=1)
                exe = mon.upload_program(file)
                continue

            if cmd.startswith("upload "):
                cmd_args = cmd.split(' ')
                file = cmd_args[1]
                address = int(cmd_args[2], 16) if len(cmd_args) > 2 else 0x80090000
                mon.upload_file(file, address)
                continue
